...
int parse_video_size(const char *str){
...
int parse_options(int argc, char **argv){
	...
	if (check_yuv == true){
		...
		if (strcmp(argv[2], "-video_size"))
		{
			av_log(NULL, AV_LOG_ERROR, "if you want to encode rawvideo files, need video_size option(input -video_size)\n ");
			ret = -1;
			goto fail;
		}
		...
		ret = parse_video_size(argv[3]);
		if (ret < 0){
			av_log(NULL, AV_LOG_ERROR, "check your video_size format [ex)4096x2048]\n ");
			goto fail;
		}
		
		file_out = (char *)malloc(sizeof(char)*strlen(argv[4]) + 1);
...
int open_input_file()
{
	...
	*/
	...
	if (check_yuv == 1){
		...
		inFmtCtx->streams[streamIdx]->codec->width = clip_width;
		inFmtCtx->streams[streamIdx]->codec->height = clip_height;
...
AVStream *add_stream(AVStream *instream, enum AVCodecID codec_id, AVCodec **codecDecode)
{
	...
	if ((*codecDecode)->type == AVMEDIA_TYPE_VIDEO)
	{
		...
		ctxDecode->sample_aspect_ratio.den = 1;
		ctxDecode->sample_aspect_ratio.num = 1;

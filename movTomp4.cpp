...
AVStream *add_stream(AVFormatContext *oc, enum AVCodecID codec_id)
{
	...
	if ((codecDecode)->type == AVMEDIA_TYPE_VIDEO)
	{
		...
		* identical to 1. */
		//ctxDecode->time_base = pFormatCtx->streams[videoStream]->codec->time_base;
		ctxDecode->time_base = { 1, 30};
...
void encode_init()
{
	...
	encoderOut = (uint8_t*)malloc(clip_height*clip_width * 3/2);
...
void get_infor()
{
	...
	//pFormatCtx->streams[0]->codec->width = clip_width;
	//pFormatCtx->streams[0]->codec->height = clip_height;

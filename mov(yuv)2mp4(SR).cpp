...
const char *inFilename = "C:/ffmpeg/yuvfile.mov";
const char *outFilename = "C:/ffmpeg/yuvfile.mp4";
...
int open_output_file()
{
	...
	for (i = 0; i < inFmtCtx->nb_streams; i++)
	{
		...
		if (decCtx->codec_type == AVMEDIA_TYPE_VIDEO)
		{
			...
			encCtx->bit_rate = 40 * 1000 * 1024;			// average bitrate
			encCtx->height = decCtx->height;				// resolution
			encCtx->width = decCtx->width;
			encCtx->pix_fmt = encoder->pix_fmts[0];
			outStream->time_base.num = 1;					// video time_base 
			outStream->time_base.den = FPS;
			encCtx->time_base = outStream->time_base;
...
int encode_write_frame(AVFrame *frame, int streamIdx, int *gotFrame)
{
	...
	av_log(NULL, AV_LOG_INFO, "Encoding frame %d\n", cnt);
	cnt++;
	//Encode
	...
	if (outFmtCtx->oformat->flags & AVFMT_RAWPICTURE) {
		...
		enc_pkt.stream_index = outFmtCtx->streams[streamIdx]->index;
		enc_pkt.data = (uint8_t *)frame;
		enc_pkt.size = sizeof(AVPicture);

		enc_pkt.pts = enc_pkt.dts = frame->pts;
...
int main()
{
	...
	buffer = (uint8_t*)malloc(3 * buff_size / 2);

	reFrame = av_frame_alloc();
	reFrame->format = PIX_FMT_YUV420P;
	reFrame->width = WIDTH;
	reFrame->height = HEIGHT;

	avpicture_alloc((AVPicture *)reFrame, PIX_FMT_YUV420P, WIDTH, HEIGHT);

	reFrame->data[0] = buffer;
	reFrame->data[1] = reFrame->data[0] + buff_size;
	reFrame->data[2] = reFrame->data[1] + buff_size / 4;
	reFrame->linesize[0] = outFmtCtx->streams[videoIdx]->codec->width;
	reFrame->linesize[1] = outFmtCtx->streams[videoIdx]->codec->width / 2;
	reFrame->linesize[2] = outFmtCtx->streams[videoIdx]->codec->width / 2;
	...
	while (av_read_frame(inFmtCtx, &pkt) >= 0)
	{
		...
		if (mediaType == AVMEDIA_TYPE_VIDEO) {
			...
			if (gotFrame)
			{
				...
				sws_scale(swsCtx, (const uint8_t * const *)frame->data, frame->linesize, 0, HEIGHT, reFrame->data, reFrame->linesize);
				reFrame->pts = pts;

				ret = encode_write_frame(reFrame, streamIdx, &gotFrame);
				if (ret < 0)
				{
	...
	if (outFmtCtx && outFmtCtx->streams[videoIdx] && outFmtCtx->streams[videoIdx]->codec)
